...
int main() {
	...
	std::uniform_real_distribution<double> distr_dense_net(-MAX_WEIGHT, MAX_WEIGHT);
	...
	weights1->imbue( [&]() {return distr_dense_net(engine); } );
	engine.seed(rd());
	weights2->imbue( [&]() {return distr_dense_net(engine); } );
	engine.seed(rd());
	weights3->imbue( [&]() {return distr_dense_net(engine); } );
